...
const registerEntityCrud = (
  ...
  [ROUTES.CREATE, ROUTES.CREATE_STAKE].forEach((base) => {
    registry.registerPath({
      ...
      responses: {
        [HTTP_STATUS.OK]: { description: `The created ${segment}`, content: jsonContent(dtoSchema) },
        ...badRequestResponse,
      },
    });
  });
  ...
  [ROUTES.UPDATE, ROUTES.UPDATE_STAKE].forEach((base) => {
    registry.registerPath({
      ...
      responses: {
        [HTTP_STATUS.OK]: { description: `The updated ${segment}`, content: jsonContent(dtoSchema) },
        ...badRequestResponse,
      },
    });
  });
...
ENTITY_DEFINITIONS.forEach((definition) => {
  registerEntityCrud(definition.segment, definition.createSchema, definition.updateSchema, definition.dtoSchema);
});
...
registry.registerPath({
  ...
  responses: {
    [HTTP_STATUS.OK]: { description: 'The navigation route', content: jsonContent(RetrieveNavigationRouteDtoSchema) },
    ...notFoundResponse,
  },
});
...
[ROUTES.UPDATE, ROUTES.UPDATE_STAKE].forEach((base) => {
  registry.registerPath({
    ...
    responses: {
      [HTTP_STATUS.OK]: { description: 'The updated entity', content: jsonContent(BasicEntityDtoSchema) },
      ...badRequestResponse,
    },
  });
  ...
  registry.registerPath({
    ...
    responses: {
      [HTTP_STATUS.OK]: { description: 'The updated entities', content: jsonContent(z.array(BasicEntityDtoSchema)) },
      ...badRequestResponse,
    },
  });
});
