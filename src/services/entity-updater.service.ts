...
export interface EntityUpdaterService {
  updateEntity<T extends AnyEntity>(parentId: string, entity: T, context?: TransactionContext): Promise<T>;
...
export const createEntityUpdaterService = ({
  ...
  const changeRouteWptsVisibility = async (updateDto: UpdateBasicEntityDto): Promise<void> => {
    const route = await entityRetriever.findEntityOfType<Route>(ENTITY_TYPES.ROUTE, updateDto.entity.general.id);
    if (!route) {
      throw new BadRequestError(ERROR_MESSAGES.CHANGE_VISIBILITY_MISSING_ENTITY(updateDto.entity.general.id));
    }
  ...
  const changeEntityVisibility = async (updateDto: UpdateBasicEntityDto): Promise<BasicEntityDto> => {
    ...
    try {
      ...
      const entity = await entityRetriever.findEntity(entityDto.general.id);
      if (!entity) {
        throw new BadRequestError(ERROR_MESSAGES.CHANGE_VISIBILITY_MISSING_ENTITY(entityDto.general.id));
      }
      const updated = withUpdatedTimestamp({ ...entity, isVisible: updateDto.isVisible });
      await updateEntity(entity.parentId, updated);
      return toBasicEntityDto(updated);
    } catch (error) {
      throw new BadRequestError(
        `${ERROR_MESSAGES.CHANGE_VISIBILITY_ENTITY_FAILED(entityDto.general.name)}: ${(error as Error).message}`,
      );
    }
  };

  const changeEntitiesVisibility = async (updateDto: UpdateBasicEntitiesDto): Promise<BasicEntityDto[]> => {
    logger.info({ count: updateDto.entities.length }, 'Attempting to change visibility for entity list');
    try {
  ...
  return {
    ...
    updateByType: async (entityType, dto) => {
      logger.info({ entityType, id: dto.id, parentId: dto.parentId }, 'Attempting to update entity');
      const definition = getEntityDefinition(entityType);
      const entity = await entityRetriever.findEntityOfType(entityType, dto.id);
      if (!entity) {
        throw new BadRequestError(ERROR_MESSAGES.ENTITY_UPDATE_NOT_FOUND(entityType, dto.id, dto.parentId));
      }
